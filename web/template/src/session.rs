...
//! message{% if auth != "none" %} and the id of the logged in user{% endif %}.
...
pub const COOKIE: &str = "{{crate_name}}_session";
{%- if sqlx %}
...
{%- if store == "postgres" %}
pub type SessionStore = tower_sessions_sqlx_store::PostgresStore;
{%- elsif store == "sqlite" %}
pub type SessionStore = tower_sessions_sqlx_store::SqliteStore;
{%- else %}
pub type SessionStore = tower_sessions::MemoryStore;
{%- endif %}
...
/// A session store{% if sqlx %} sharing the connection pool of `store`, with its
/// table created if missing{% endif %}.
